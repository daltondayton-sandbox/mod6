...
template <class T, class U> void checkTest(string test, T is, U expected) {
  total_tests++;
  if (is == expected) {
    correct_tests++;
...
int main() {
  ...
  et->setExpression(expression);
  et->printParseTreePostOrder(out);
  checkTest("Test 18", out.str(),
            "9 2 110 30 2 / - * + 8 * 1000 + 2 / 3 4 ^ 1 + 2 / +");
